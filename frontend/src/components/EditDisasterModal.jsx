...
const EditDisasterModal = ({ isOpen, disaster, onClose, onUpdateDisaster }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onUpdateDisaster(disaster.id, disasterData);
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-dark-card rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-dark-text">Edit Disaster</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {isSubmitting ? 'Updating...' : 'Update Disaster'}
            </button>
          </div>
        </form>
      </div>
